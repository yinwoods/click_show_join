...
def main(separator='\t'):
    ...
    input_1_schema = read_schema('1')
    input_2_schema = read_schema('2')
    input_3_schema = read_schema('3')
    ...
    for key1, group in groupby(data, itemgetter(0, 1)):
        ...
        for line in group:
            ...
            show_or_click_data[key].setdefault(line_type, dict(zip(eval('input_%s_schema' % line_type), line[5:])))

        for key, values in show_or_click_data.items():
            if key == 'zan':
                continue
            if values.get('1', None) is not None:
                if values.get('2', None) is not None and show_or_click_data.get('zan', None) is not None:
                    values['3'] = show_or_click_data['zan']['3']
                    del show_or_click_data['zan']
                output_data = []
                for (schema, suffix, default) in output_schema:
                    if suffix != '0':
                        output_data.append(values.get(suffix, dict()).get(schema, default))
                    else:
                        output_data.append('0' if values.get('2', None)  is None else '1')
